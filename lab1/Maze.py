...
class LifeLikeAutomaton:
    ...
    def parse_rule(rule_str: str):
        rule_str = rule_str.strip().upper().replace(" ", "")
        parts = rule_str.split("/")
        if len(parts) != 2:
            raise ValueError('Правило має бути у форматі "B.../S...", наприклад B3/S1234')
        b_part, s_part = parts
        if not b_part.startswith("B") or not s_part.startswith("S"):
            raise ValueError('Правило має мати вигляд "B.../S..." (наприклад B3/S1234)')
        B = set(int(ch) for ch in b_part[1:] if ch.isdigit())
        S = set(int(ch) for ch in s_part[1:] if ch.isdigit())
        for v in B | S:
            if not (0 <= v <= 8):
                raise ValueError("Цифри у B та S мають бути в діапазоні 0-8")
        return B, S

    def set_rule(self, rule_str: str):
        B, S = self.parse_rule(rule_str)
        self.B, self.S = B, S
    ...
    def step(self):
        ...
        born_mask = (~alive_mask) & (np.isin(neighbor_count, list(self.B)) if self.B else np.zeros_like(alive_mask))
        survive_mask = alive_mask & (np.isin(neighbor_count, list(self.S)) if self.S else np.zeros_like(alive_mask))

        self.grid = (born_mask | survive_mask).astype(np.int32)
        self.iteration += 1
    ...
    def randomize(self, density: float = 0.04):
    ...
    def center_square(self, half_size: int = 3):
        self.grid = np.zeros((self.height, self.width), dtype=np.int32)
        cy, cx = self.height // 2, self.width // 2
        self.grid[cy - half_size:cy + half_size + 1, cx - half_size:cx + half_size + 1] = 1
        self.iteration = 0
        self.initial_grid = self.grid.copy()
    ...
    def render_rgb_array(self) -> np.ndarray:
        """0 -> чорний фон, 1 -> яскраво-бірюзова стінка лабіринту."""
        rgb = np.zeros((self.height, self.width, 3), dtype=np.uint8)
        rgb[self.grid == 1] = (80, 220, 210)
        return rgb
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title('Клітинний автомат "Лабіринт" (B/S-правило)')
        self.configure(bg="#1e1e1e")
        self.resizable(False, False)

        self.automaton = LifeLikeAutomaton(GRID_SIZE, GRID_SIZE, DEFAULT_RULE)
    ...
    def _build_ui(self):
        ...
        tk.Label(right, text="Лабіринт", **style_title).pack(anchor="w")
        ...
        rule_frame = tk.LabelFrame(right, text="Правило (B.../S...)", bg="#1e1e1e", fg="white",
        ...
        tk.Button(init_frame, text="Центральний квадрат (запальник)",
        ...
        tk.Button(init_frame, text="Розріджене випадкове поле",
                  command=self._on_random).pack(fill="x", padx=6, pady=(2, 6))
    ...
    def _update_status(self):
        ...
        self.rule_info_label.config(text=f"B={{{B}}}  S={{{S}}}")
    ...
    def _on_save_bmp(self):
        path = filedialog.asksaveasfilename(
            ...
            initialfile=f"maze_iter_{self.automaton.iteration}.bmp",
